...
const REMOTE_DB_URL = 'https://xn--80adfeardlcc5bxinj.xn--p1ai/upload/db.json';
// Local Python Backend URL
const LOCAL_API_URL = 'http://localhost:8000/api/db';
...
export const loadDatabase = async (): Promise<Database> => {
    ...
    const remoteUrl = `${REMOTE_DB_URL}?v=${timestamp}`;

    console.groupCollapsed("🔍 Database Connection Strategy");

    // 0. Try Local Python Backend (SQLite)
    try {
        // Short timeout for local check to not delay startup if not running
        ...
        if (response.ok) {
            ...
            console.log("✅ Loaded from Python Backend (SQLite)");
            console.groupEnd();
            return data;
        }
    } catch (e) {
        console.log("⚠️ Python Backend (localhost:8000) not found, proceeding to fetch methods...");
    }
    ...
    try {
        ...
        if (response.ok) {
            const data = await response.json();
            console.log("✅ Loaded via Direct Fetch (Remote)");
            console.groupEnd();
            return data;
        }
    } catch (e) {
        console.log("⚠️ Direct remote fetch failed, trying proxies...");
    }
    ...
    for (const proxy of proxies) {
        try {
            ...
            const response = await fetch(proxy(remoteUrl), { signal: controller.signal });
            clearTimeout(id);
